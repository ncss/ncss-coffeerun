...
def edit_coffee(coffeeid):
    ...
    runs = Run.query.filter(Run.time >= sydney_timezone(datetime.utcnow())).all()
...
def add_run():
    ...
    form.status.choices = [(s.id, s.description) for s in statuses]
    if request.method == "GET":
        form.time.data = sydney_timezone(datetime.utcnow())#.strftime("%Y/%m/%d %H:%M:%S")
        form.deadline.data = sydney_timezone(datetime.utcnow())
...
def add_coffee(runid=None):
    runs = Run.query.filter(Run.deadline >= sydney_timezone(datetime.utcnow())).filter(Run.status==1).all()
    if not runs:
