...
class WrappedSlackBot:
  ...
  def __init__(self, token, user_id, team_id):
    ...
    self.ORDERS_DISPATCH[re.compile('(?:(?:open|list) )?runs')] = self.list_runs
    self.ORDERS_DISPATCH[re.compile('order(?: an?)? ([^\=]+)(?: run=(?P<runid>[0-9]+))?')] = self.order_coffee
    self.ORDERS_DISPATCH[re.compile('([^\=]+) (?:plz|pls|please|plox)(?: run=(?P<runid>[0-9]+))?')] = self.order_coffee
